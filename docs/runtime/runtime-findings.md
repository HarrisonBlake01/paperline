...
| RT-10 | High launch config | Production alias is `paperline-xi.vercel.app`; intended `paperline.io` is unresolved in docs, and prior preview displayed Clerk development-key warnings. | Select canonical domain; align Clerk URLs/keys, app URL, webhooks, email links, DNS, CSP, and MCP host policy. | Dashboard and immutable candidate verification. | **Blocked** pending explicit dashboard/domain approval. |
